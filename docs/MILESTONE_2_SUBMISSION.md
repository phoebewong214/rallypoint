...
exposing **59 endpoints** (+ `/health`) over **17 database tables**, with:
...
- **Full feature set**: partner matching, saved players, courts with real
  ...
  invites, date-specific availability overrides, profile photos, an admin
  dashboard, trust & safety reports, and a support desk.
...
|---|---:|---|---|
| **API List with Expected Input/Output + Test Case Documentation** | 20 | Complete endpoint list with auth, expected input, and sample JSON I/O for all 59 routes | **[MILESTONE_2_API_REFERENCE.md](./MILESTONE_2_API_REFERENCE.md)** + interactive Swagger at `/api/docs/` |
...
- [x] **Finalized backend APIs** to support the capstone solution — 59 endpoints.
