...
| `test_profile.py` | 13 | Profile PATCH, sport-profile upsert/remove, availability grid replace, dated availability overrides, profile-photo upload/removal |
...
python api_demo.py > ../milestone2/api_demo_output.txt
```
...
The run also covers the two newest features: a **dated availability override**
(`PATCH /api/auth/me` → `availability_overrides 0→1`) and **profile photos** — a
base64 data-URL upload (`PUT /api/auth/me/photo` → `user_photos 0→1`) that an
admin later removes as a moderation action (`user_photos 1→0`).

The complete transcript is `milestone2/api_demo_output.txt`. Highlights follow.
...
| 7 | Dated availability override | 200 | `availability_overrides 0→1 (+1)` — busy one concrete morning despite the weekly grid |
| 8 | Upload profile photo (data URL) | 200 | `user_photos 0→1 (+1)`; response bumps `photoVersion` |
| 9 | GET /players (AI ranked) | 200 | none (read; score computed inline) |
| 10 | Save player | 200 | `saved_players 0→1 (+1)` |
| 11 | Un-save player | 200 | `saved_players 1→0 (−1)` |
| 12 | Report player | 201 | `user_reports 0→1 (+1)` |
| 13 | AI match-reason | 200 | `ai_match_logs 0→1 (+1)` |
| 14 | AI match-reason (again) | 200 | none — **upsert**, no duplicate row |
| 15 | Create session (request) | 201 | `sessions 0→1 (+1)`, `status=pending` |
| 16 | Duplicate request | 409 | none — returns the existing session |
| 17 | Accept session | 200 | row: `status pending → confirmed` |
| 18 | Reschedule | 200 | row: `host/guest swapped`, new time, `status → pending` |
| 19 | GET /sessions | 200 | none (read) |
| 20 | Cancel session | 200 | row: `status → cancelled` (kept as a trace) |
| 21 | GET /courts | 200 | none (read; real distance) |
| 22 | Favorite court | 200 | `court_favorites 0→1 (+1)` |
| 23 | Check in | 200 | `court_checkins 0→1 (+1)` |
| 24 | Create open game | 201 | `court_appointments 0→1 (+1); appointment_participants 0→1 (+1)` |
| 25 | Maya joins | 200 | `appointment_participants 1→2 (+1)` |
| 26 | Casey joins (full) | 200 | `appointment_participants 2→3 (+1)` — **waitlisted** |
| 27 | Maya leaves | 200 | `appointment_participants 3→2 (−1)` — Casey **promoted** off waitlist |
| 28 | Check out | 200 | `court_checkins 1→0 (−1)` |
| 29 | Create invite (window) | 201 | `game_invites 0→1 (+1); time_proposals 0→1 (+1)` |
| 30 | Confirm opponent | 200 | row: `phase awaiting_opponent → settling_time` |
| 31 | Propose specific time | 200 | `time_proposals 1→2 (+1)` (old one superseded) |
| 32 | Accept time | 200 | `sessions 1→2 (+1)`; invite row: `phase → confirmed, session_id → 2` |
| 33 | Escalate support | 200 | `support_tickets 0→1 (+1)` |
| 34 | Admin overview | 200 | none (read aggregate) |
| 35 | Admin list users | 200 | none (read) |
| 36 | Admin list reports | 200 | none (read) |
| 37 | Admin review report + suspend | 200 | report row: `status open → reviewed`; user `is_active 1 → 0` |
| 38 | Admin removes user's photo (moderation) | 200 | `user_photos 1→0 (−1)` — back to the initials avatar |
...
```
STEP 17: Maya accepts the request (UPDATE sessions status -> confirmed)
...
```
STEP 13: On-demand AI match reason, cached (INSERT ai_match_logs)
  DB DELTA: ai_match_logs 0->1 (+1 row)
STEP 14: Same call again — upserts the cached row (no new row)
  DB DELTA: (no row-count change — in-place UPDATE or read-only)
```

**MATERIALIZE — accepting an invite time creates a confirmed session:**
```
STEP 32: Alex accepts the time -> materializes a confirmed session
...
```
STEP 37: Admin reviews the report + suspends the account
...
|---|---|---|
| Automated tests | `cd backend && pytest -v` | `124 passed` → `milestone2/pytest_output.txt` |
| Live DB-evidence demo | `cd backend && python api_demo.py` | `milestone2/api_demo_output.txt` |
| Interactive API docs | `python app.py` then open `/api/docs/` | Swagger UI |
| Schema DDL | generated by the models | `milestone2/schema_ddl.sql` |
