...
exposing **60 endpoints** over **17 database tables**, with:
...
- **Full feature set**: partner matching, saved players, profile photos
  (DB-blob avatars with admin moderation), weekly availability plus
  date-specific overrides, courts with real Haversine distance, check-ins, open
  games with a waitlist, two-phase game invites, an admin dashboard, trust &
  safety reports, and a support desk.
...
|---|---:|---|---|
| **API List with Expected Input/Output + Test Case Documentation** | 20 | Complete endpoint list with auth, expected input, and sample JSON I/O for all 60 routes | **[MILESTONE_2_API_REFERENCE.md](./MILESTONE_2_API_REFERENCE.md)** + interactive Swagger at `/api/docs/` |
| **Database Design and Implementation** | 20 | 17-table schema: ER diagram, per-table columns/types/keys/constraints, indexes, design rationale, generated DDL | **[MILESTONE_2_DATABASE_DESIGN.md](./MILESTONE_2_DATABASE_DESIGN.md)** + [schema_ddl.sql](./schema_ddl.sql) |
| **API Testing Results and Live demonstration** | 60 | 124 passing automated tests + a 39-call live run showing the DB change after **each** operation | **[MILESTONE_2_TEST_RESULTS.md](./MILESTONE_2_TEST_RESULTS.md)** + [pytest_output.txt](./pytest_output.txt) + [api_demo_output.txt](./api_demo_output.txt) |

Assignment requirements checklist:

- [x] **Finalized backend APIs** to support the capstone solution — 60 endpoints.
- [x] **List of APIs with expected input/output + sample JSON** — API reference.
- [x] **Database design and tables** — 17 tables, ER diagram, DDL.
- [x] **Test case documents and test results** — 124 tests + case table.
...
# 1. Automated tests  → 124 passed
pytest -v

# 2. Live DB-evidence demo  → request/response/DB-delta for 39 calls
python api_demo.py
...
| Tests | pytest + pytest-flask (124 cases, CI on every push) |
